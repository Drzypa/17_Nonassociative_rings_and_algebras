...
\usepackage{pmmeta}
\pmcanonicalname{KillingForm}
\pmcreated{2013-03-22 13:15:47}
\pmmodified{2013-03-22 13:15:47}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{Killing form}
\pmrecord{8}{33743}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $\fr g$ be a finite dimensional Lie algebra over a field $k$, and $X,Y\in\fr g$.  Let $\ad_X:\fr g\to\fr g$ be the adjoint action, 
$\ad_XY=[X,Y]$. 

Then the Killing form on $\fr g$ is a bilinear map $$B_{\fr g}:\fr g\times\fr g\to k$$ given by $$B_{\fr g}(X,Y)=\tr(\ad_X\circ\ad_Y).$$

The Killing form is \PMlinkname{invariant}{InvariantFormLieAlgebras} under the adjoint action and symmetric (since trace is
symmetric).
%%%%%
%%%%%
\end{document}
