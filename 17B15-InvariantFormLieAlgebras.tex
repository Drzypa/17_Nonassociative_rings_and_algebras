...
\usepackage{pmmeta}
\pmcanonicalname{InvariantFormLieAlgebras}
\pmcreated{2013-03-22 13:15:45}
\pmmodified{2013-03-22 13:15:45}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{invariant form (Lie algebras)}
\pmrecord{5}{33742}
\pmprivacy{1}
\pmauthor{bwebste}{988}
\pmtype{Definition}
\pmcomment{trigger rebuild}
\pmclassification{msc}{17B15}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $V$ be a representation of a Lie algebra $\fr g$ over a field $k$. Then a bilinear form 
$B:V\times V\to k$ is {\em invariant} if $$B(Xv,w)+B(v,Xw)=0.$$
for all $X\in\fr g, v,w\in V$.  This criterion seems a little odd, but in the context of Lie algebras,
it makes sense.  For example, the map $\tilde{B}:V\to V^*$ given by $v\mapsto B(\cdot,v)$
is equivariant if and only if $B$ is an invariant form.
%%%%%
%%%%%
\end{document}
