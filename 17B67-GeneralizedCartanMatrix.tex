...
\usepackage{pmmeta}
\pmcanonicalname{GeneralizedCartanMatrix}
\pmcreated{2013-03-22 13:52:56}
\pmmodified{2013-03-22 13:52:56}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{generalized Cartan matrix}
\pmrecord{5}{34625}
\pmprivacy{1}
...
\pmrelated{ExtendedCartanMatrix}
\pmdefines{symmetrizable}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
A {\em generalized Cartan matrix} is a matrix $A$ whose diagonal entries are all 2, and whose off-diagonal entries are nonpositive integers, such that $a_{ij}=0$ if and only if $a_{ji}=0$.  Such a matrix is called {\em symmetrizable} if there is a diagonal matrix $B$ such that $AB$ is symmetric.
%%%%%
%%%%%
\end{document}
