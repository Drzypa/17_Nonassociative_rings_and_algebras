...
\usepackage{pmmeta}
\pmcanonicalname{KacMoodyAlgebra}
\pmcreated{2013-03-22 13:31:52}
\pmmodified{2013-03-22 13:31:52}
\pmowner{bwebste}{988}
\pmmodifier{bwebste}{988}
\pmtitle{Kac-Moody algebra}
\pmrecord{7}{34124}
\pmprivacy{1}
...
\newcommand{\Zp}{\Z/p}
\begin{document}
Let $A$ be an $n\times n$ generalized Cartan matrix.  If $n-r$ is the rank of $A$, then let $\fr{h}$ be a $n+r$ dimensional complex vector space.  Choose $n$ linearly independent elements $\alpha_1,\ldots,\alpha_n\in\fr{h}^*$ (called \emph{roots}), and $\check{\alpha}_1,\ldots,\check{\alpha}_n\in\fr{h}$ (called \emph{coroots}) such that $\inn{\alpha_i,\check{\alpha_j}}=a_{ij}$, where $\inn{\cdot,\cdot}$ is the natural pairing of $\fr{h}^*$ and $\fr{h}$. This choice is unique up to automorphisms of $\fr{h}$.
...
\begin{align*}
[X_i,Y_i]&=\check{\alpha_i} &[X_i,Y_j]&=0\\
[X_i,h]&=\alpha_i(h)X_i&[Y_i,h]&=-\alpha_i(h)Y_i\\
\underbrace{[X_i,[X_i,\cdots,[X_i}_{1-a_{ij} \text{ times}},X_j]\cdots]]&=0&
\underbrace{[Y_i,[Y_i,\cdots,[Y_i}_{1-a_{ij} \text{ times}},Y_j]\cdots]]&=0
\end{align*}
for any $h\in\fr{h}$.
